...
class Drive360(Dataset):
    ...
    # phase (string) to specify either 'train', 'test', 'validation'
    def __init__(self, config, phase):
        self.config = config
        ...
        self.dataframe = pd.read_csv(
            os.path.join(self.data_dir, self.csv_name),
            dtype={
                'cameraFront': object,
                'cameraRear': object,
                'cameraRight': object,
        ...
        # boundaries but would still like to obtain a temporal sequence of images, we cannot start at index 0 of each
        # chapter
...
def test():
    ...
    loader = Drive360Loader(config, 'validation')
    print('Loaded val loader with the following data available as a dict.')
    print(loader.drive360.dataframe.keys())
    for i, batch in enumerate(loader):
        print(i, len(batch), batch[0].keys(), batch[0]['cameraFront'].keys())
        if i > 1:
            break
