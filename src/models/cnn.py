...
class SlowFusionModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        towers_out = {camera: {frame: self.towers[camera].get(frame)(x) for frame in frames}
        ...
        for camera in self.cameras:
            for frame in range(self.num_frames_per_camera - 1):
                ...
                frame_pair = str(frame) + str(frame + 1)
                early_fusions_out[camera][frame_pair] = \
                    self.early_fusions[camera].get(frame_pair)(frames)
        ...
        for camera in self.cameras:
            ...
            middle_fusions_out[camera] = \
                self.middle_fusions[camera](frames_pairs)
    ...
    def _make_early_fusion(self) -> nn.Module:
        return nn.Sequential(OrderedDict({
            ...
            'block3': self._make_block(128, 128),
        }))
    ...
    def _make_middle_fusion(self) -> nn.Module:
        ...
        return nn.Sequential(OrderedDict({
            ...
            'pool': nn.MaxPool2d(kernel_size=2, stride=2),
            'block4': self._make_block(128, 128),
        }))
    ...
    def _make_late_fusion(self) -> nn.Module:
        ...
        return nn.Sequential(OrderedDict({
            ...
            'block3': self._make_block(256, 256),
            'pool': nn.MaxPool2d(kernel_size=2, stride=2),
